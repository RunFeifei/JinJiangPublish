...
public class FileUtils {
     ...
     * @return 图片地址
     */
    public static File createAppPath() {
        ...
        File file = new File(directoryPath, "JinJiangPublish");
        if (!file.exists()) {
            file.mkdirs();
        }
        return file;
    ...
    public static String createExternalPicturePath() {
        File file = new File(createAppPath(), Environment.DIRECTORY_PICTURES);
        if (!file.exists()) {
            file.mkdirs();
        }
        return file.getPath();
    ...
    public static String html2File(String title, String html) {
        try {
            String externalDocumentsPath = createExternalDocumentsPath();
            String fileName = externalDocumentsPath + File.separator + title + ".html";
